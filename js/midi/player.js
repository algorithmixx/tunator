...
midi.start =
midi.resume = function(onsuccess,onQueue) {
    if (midi.currentTime < -1) {
    	midi.currentTime = -1;
    }
    startAudio(midi.currentTime, null, onsuccess, onQueue);
};
...
midi.loadMidiFile = function(onsuccess, onprogress, onerror) {
	try {
		...
		///
		MIDI.loadPlugin({
			instruments: midi.getFileInstruments(),	// hopefully we will find all instruments in our font base , GS
			onsuccess: onsuccess,
...
//				console.log(event.channel, MIDI.defineControl[event.controllerType], event.value);
				break;
			case 'programChange':
				MIDI.programChange(event.channel, event.programNumber); // GS... 
...
var scheduleTracking = function(channel, note, currentTime, offset, message, velocity, bend) {
...
var startAudio = function(currentTime, fromCache, onsuccess, onQueue) {
	if (!midi.replayer) {
		return;
	}
	if (!fromCache) {
	...
	for (var n = 0; n < length && messages < 100; n++) {
		...
		switch (event.subtype) {
			...
			case 'pitchBend':
				...
				break;
			case 'noteOn':
				event.bend=0;	// default value
				if (channel.mute) break;
				note = event.noteNumber - (midi.MIDIOffset || 0);
				// invoke callback for pitch shifting
				if(onQueue) onQueue(event,queuedTime + midi.startDelay);
				eventQueue.push({
				    event: event,
				    time: queueTime,
				    source: MIDI.noteOn(channelId, event.noteNumber, event.velocity, delay,event.bend),
				    interval: scheduleTracking(channelId, note, queuedTime + midi.startDelay, offset - foffset, 144, event.velocity, event.bend)
				});
				messages++;
				break;
			case 'noteOff':
				...
				eventQueue.push({
				    ...
				    interval: scheduleTracking(channelId, note, queuedTime, offset - foffset, 128, event.velocity, event.bend)
				});
				break;
			default:
				break;
