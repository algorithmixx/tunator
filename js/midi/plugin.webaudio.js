...
(function(root) { 'use strict';
	...
	window.AudioContext && (function() {
		...
		var audioBuffers = {};
		var loopingLongNotes=false;
		///
		midi.audioBuffers = audioBuffers;
		midi.send = function(data, delay) { };
		midi.setController = function(channelId, type, value, delay) { };
		
		midi.loopLongNotes = function (loop) {
			loopingLongNotes=loop;
		}
...
// 			} else {
				var channel = root.channels[channelId];
				channel.pitchBend = program;
				console.log("set pitchBend ("+channelId+") to "+program);
// 			}
		};

		midi.noteOn = function(channelId, noteId, velocity, delay, bend) {
			delay = delay || 0;
...
// 				console.log(MIDI.GM.byId[instrument].id, instrument, channelId);
			...
			source.loop= loopingLongNotes;	// in case a midi note lasts longer than the buffer
			...
			source.playbackRate.value = 1 + (bend ? bend : 0); // GS ... // pitch shift 
